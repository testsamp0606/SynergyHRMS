...
>(
  (
    ...
    // The isMobile check might cause hydration issues, so we'll render null on the server.
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7", isMobile && 'md:hidden', !isMobile && 'hidden md:flex', className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[variant=inset]:min-h-[calc(100screen-theme(spacing.4))] md:peer-data-[variant=inset]:m-2 md:peer-data-[state=collapsed]:peer-data-[variant=inset]:ml-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow",
        className
      )}
      {...props}
    />
